...
Our pages layout mostly resumes to correctly assembling the modules layout. Also, the few page-level callbacks are designed to ensure the interaction between the different modules: forward the `ImportExportBlueprint` data to the `FormBlueprint` when the user imports data and reciprocally when the user exports the form state, turn the `FormBlueprint` results into metrics to be displayed in the `MetricsBlueprint`...
